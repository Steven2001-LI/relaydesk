...
**实测数据**：RAG 在弱检索器(BM25)下 rerank 把 Hit@1 87.3%→92.7%、MRR 0.873→0.927；端到端答案质量（DeepSeek-judge）准确性 4.92 / 有用性 4.85（满分 5）；工具调用基线集 22–24/24（LLM 非确定，两条技术样本在“先查服务状态”上摇摆）、样本偏易已饱和，对抗集 13/15，暴露 3 类缺口：工具层无鉴权/归属校验、显式“别查系统”仍误触发、条件多意图未自动编排。
