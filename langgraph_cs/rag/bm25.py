...
数据同源（关键，保证评测公平）：
  BM25 索引建在与向量库**完全相同**的 121 个条目 chunk 上——直接复用 store.load_faq_documents()
  解析出的 Document（同样的 page_content=问题+答案、同样的 metadata={source,item_id}）。
  绝不在这里重写一份 faq 解析逻辑（遵循 code-reuse）。这样 dense 与 bm25 两条链路吃的是
...
def _tokenize(text: str) -> list[str]:
    ...
    这里用最朴素的精确模式分词，不做停用词过滤——教学项目，越透明越好；
