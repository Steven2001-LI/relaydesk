...
为什么持久目录单独放 langgraph_cs/data/chroma_rag/？
  避免污染其他项目的 data/chroma（可能是另一套 collection 与 embedding）。
...
# Chroma 持久化目录（单独一份，不碰其他项目的 data/chroma）。
...
def load_faq_documents(docs_dir: Path | str = DEFAULT_DOCS_DIR) -> list[Document]:
    ...
    都复用它，保证两条检索链路吃的是**完全相同的条目 chunk**（同样的 page_content
    与 metadata），评测对比才公平：解析逻辑只此一份，绝不复制。
...
def _reset_collection() -> None:
    """
    重建前先清掉旧 collection，避免与上一版（粗切结果）混在一起。
