...
| refund-create-01 | billing | billing_agent | create_refund_ticket({"order_id": "ORD-20260508-004", "reason": "*", "user_id": "user_004"}) | create_refund_ticket({"order_id": "ORD-20260508-004", "reason": "收到的空气净化器外壳有裂痕，存在质量问题", "user_id": "user_004"}) → refund_status({"order_id": "ORD-20260508-004"}) | True | True | True | OK |
| refund-create-02 | billing | billing_agent | create_refund_ticket({"order_id": "ORD-20260520-010", "reason": "*", "user_id": "user_010"}) | create_refund_ticket({"order_id": "ORD-20260520-010", "reason": "不想要了，订单未发货，申请仅退款", "user_id": "user_010"}) → refund_status({"order_id": "ORD-20260520-010"}) | True | True | True | OK |
...
| tech-ticket-01 | technical | technical_agent | create_ticket({"detail": "*", "user_id": "user_004"}) | check_service_status({}) → create_ticket({"detail": "智能门锁App蓝牙一直配对失败，无法连接门锁。用户已确认手机蓝牙已开启，但仍提示配对失败，需进一步排查。", "user_id": "user_004"}) | True | True | True | OK |
| tech-ticket-02 | technical | technical_agent | create_ticket({"detail": "*", "user_id": "user_001"}) | check_service_status({}) → create_ticket({"detail": "智能窗帘定时场景配置失败，需要远程协助排查。用户已确认非平台服务故障。", "user_id": "user_001"}) | True | True | True | OK |
| tech-ticket-03 | technical | technical_agent | create_ticket({"detail": "*", "user_id": "user_010"}) | check_service_status({}) → create_ticket({"detail": "会议摄像头升级后仍黑屏。用户已尝试升级操作，但问题未解决，升级后摄像头依然黑屏无画面。需要技术支持进一步排查硬件或驱动问题。", "user_id": "user_010"}) | True | True | True | OK |
...
| missing-id-04 | missing_info | technical_agent | no tool | - | - | - | True | TN: no tool call |
