...
用 **LangGraph** 编排的最小可跑客服 Agent。
以 LangGraph 原生抽象实现客服编排，替代不依赖框架的手写 Orchestrator 方案。
...
整体是**多 Agent 编排**（早期形态为单 `agent_node` + 按意图换 system prompt）：
...
> **一句话结论**（评测口径：121 条 FAQ / 55 条改写 query / 条目级命中判定）：
> 在强 dense 检索器上 rerank 增益≈0（已饱和），在弱 BM25 词法检索器上 rerank 将 Hit@1 从 87.3% 提升到 92.7%、
> MRR 从 0.873 提升到 0.927，救回 3 条原本漏检的 query。rerank 价值取决于一阶段召回质量，而非默认收益。
...
| `CS_CHECKPOINT` | 后端 | 行为 |
|---|---|---|
| `memory`（默认） | `MemorySaver` | 进程内存版，零依赖、**重启即丢**（默认后端） |
...
| `nodes/intent.py` | 意图识别节点（LLM 单路，含 `escalation` 意图） | 手写三路融合识别器 |
...
| `nodes/tools.py` | billing/technical 业务工具：查账单、查退款、创建退款/技术工单、查服务大盘；工具层基于 `session_user_id` 做归属鉴权；退款工具内含 approval interrupt | 手写业务接口/服务层散落调用 |
...
| `web/static/` | 原生 HTML/JS/CSS 单页：五阶段决策轨迹、工具调用 meta、打字机、坐席模式、审批模式、在飞流 abort | （新增能力，无手写对应） |
| `scripts/seed_business_db.py` | 一键重建 mock 业务库，提供工具演示/评测用的确定性订单、账单、工单数据 | （新增能力，无手写对应） |
| `scripts/verify_persistence.py` | 离线证明 SqliteSaver 跨进程持久化（本进程写 → 子进程读回断言） | redis 会话持久化的验证 |
| `eval/tool_eval.py` | 工具调用质量评测（should-call / tool selection / args accuracy / authz security checks），含离线 self-test 与 Markdown 报告 | （新增能力，无手写对应） |
| `eval/answer_eval.py` | 端到端答案质量评测（跑图 + DeepSeek judge 打分，本地保底，不上云） | （新增能力，无手写对应） |
| `eval/langsmith_eval.py` | LangSmith trace + 数据集 + `evaluate` 端到端评测（需 key、上云） | （新增能力，无手写对应） |

### 对照手写 Orchestrator 方案

不依赖框架时需**手写**三层路由：意图路由（按意图类别选专属 Agent）、
...
| 手写实现（不用框架） | langgraph_cs（LangGraph 原生） |
|---|---|
...
- **RAG**（已完成）：在 agent 前加 `rag_node`，用 LangChain Retriever 检索知识库，做朴素 vs rerank 的指标对比（见上方「RAG 检索链路」）
- **多 Agent 路由 + HITL**（已完成）：`add_edge` 换成 `add_conditional_edges`，按意图路由到多个真正的 Agent 节点 + 失败降级（低置信 + 运行时）+ `interrupt` 实现 human-in-the-loop 转人工（见上方路由图）
- **持久化 + 可观测/评测**（已完成）：持久化（`MemorySaver` → `SqliteSaver`，`CS_CHECKPOINT` 可切换，跨进程记忆）+ 本地端到端答案质量评测（DeepSeek judge）+ LangSmith 节点级 trace/数据集/evaluate（见上方「持久化 + 可观测/评测」）
- **Web 演示**（已完成）：FastAPI + 原生前端 Web 演示，SSE 流式 token、五阶段决策轨迹、坐席/审批模式、在飞流 abort
- **工具调用 + 人工审批**（已完成）：billing/technical 工具调用、ToolNode 回流、退款创建前人工审批、mock 业务库与离线工具测试
- **工具评测 + README 收口**（已完成）：工具调用质量评测，记录 should-call / 工具选择 / 参数准确率真实数字
- **性能路由（扩展点）**：同类多 Agent 实例时按 `routing_score` 选最优（当前先不做）
