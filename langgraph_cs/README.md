...
用 **LangGraph** 搭建的最小可跑客服 Agent，用框架原生抽象表达多 Agent 编排，
重点是**讲得清每一行**的设计取舍。
...
相比「一个 `agent_node` + 按意图换 system prompt」的单节点方案，这里是**真正的多 Agent 编排**：
...
> **一句话结论**："构建 RAG 检索评测（121 条 FAQ / 55 条改写 query / 条目级命中判定），量化 rerank 的 ROI：
...
| `CS_CHECKPOINT` | 后端 | 行为 |
|---|---|---|
| `memory`（默认） | `MemorySaver` | 进程内存版，零依赖、**重启即丢**（默认行为） |
...
最新 hard 回归里 `hard-cross-user-01/02` 均通过安全检查；Web/CLI 已接入 demo 登录态：会话声明的
`session_user_id` 会注入 LangGraph config，端到端演示跨用户拦截。注意这只是客户端声明的 demo 身份、非认证；
真实部署必须由服务端从已认证会话派生身份，绝不可信客户端输入。工具层归属鉴权才是隔离机制。显式限制查询误触发通过
...
technical prompt 保持原样，避免 `tech-ticket-01/02` 回归。条件多意图（`hard-multi-intent-02`，“如果…就…”第二步为写操作 `create_refund_ticket`）不作为缺陷强修：模型查完第一单停下确认与退款人工审批门一致（安全默认），而“两步都调”时并未真正按查询结果判断条件（本例恰对）；真·条件编排如需展示需专门构建并仍走审批门。
...
| 文件 | 作用 | 备选的手写实现思路 |
|---|---|---|
| `state.py` | 图的状态 `CSState`（TypedDict + `add_messages`，含 `escalated` 标记） | 一堆 Request/Result dataclass |
| `nodes/intent.py` | 意图识别节点（LLM 单路分类，含 `escalation` 意图） | 三路加权融合的意图识别器（LLM+Embedding+关键词） |
| `nodes/agent.py` | 多个专职 Agent 节点（technical/billing/general），共享 `_run_agent` 出字 + 失败兜底 | 手写 TechnicalAgent/BillingAgent/GeneralAgent 多个 Agent 类 |
| `nodes/router.py` | 路由函数 `route_by_intent`（意图路由 + 低置信度降级） | Orchestrator 的意图/降级路由方法 |
| `nodes/tools.py` | billing/technical 业务工具：查账单、查退款、创建退款/技术工单、查服务大盘；工具层基于 `session_user_id` 做归属鉴权；退款工具内含 approval interrupt | 散落在服务层的业务接口调用 |
| `business/db.py` | mock SQLite 业务库查询/写入层（orders/bills/tickets） | 外部业务系统/数据库 |
...
| `main.py` | CLI 入口（支持 interrupt → 人工输入 → resume 循环；读 `CS_CHECKPOINT` 选后端） | 手写 CLI 入口 |
| `web/server.py` | Web 适配层：FastAPI 把图包成 `/api/chat`·`/api/resume`（SSE 流式）+ 提供静态聊天页；复用 `build_graph()`，不改图核心 | 手写 FastAPI 应用 |
| `web/static/` | 原生 HTML/JS/CSS 单页：五阶段决策轨迹、工具调用 meta、打字机、坐席模式、审批模式、在飞流 abort | （新增能力，无手写对应物） |
| `scripts/seed_business_db.py` | 一键重建 mock 业务库，提供工具演示/评测用的确定性订单、账单、工单数据 | （新增能力，无手写对应物） |
| `scripts/verify_persistence.py` | 离线证明 SqliteSaver 跨进程持久化（本进程写 → 子进程读回断言） | redis 会话持久化的验证 |
| `eval/tool_eval.py` | 工具调用质量评测（should-call / tool selection / args accuracy / authz security checks），含离线 self-test 与 Markdown 报告 | （新增能力，无手写对应物） |
| `eval/answer_eval.py` | 端到端答案质量评测（跑图 + DeepSeek judge 打分，本地保底，不上云） | （新增能力，无手写对应物） |
| `eval/langsmith_eval.py` | LangSmith trace + 数据集 + `evaluate` 端到端评测（需 key、上云） | （新增能力，无手写对应物） |

### 对照：手写 Orchestrator 备选实现

不用编排框架时，通常要在 Orchestrator 里**手写**三层路由：意图路由（按意图类别选专属 Agent）、
性能路由（同类多实例按 `routing_score()` 选最优）、降级路由（专属 Agent 不可用/失败 → 通用 Agent），
升级则往往只靠关键词检测（`转人工/人工客服/escalate/无法处理`）置一个 `escalate` 标志（**仅占位、不真正阻塞等人工**）。

langgraph_cs 用 LangGraph **原生能力**表达同一套思路，少写很多胶水代码：

| 手写实现思路 | langgraph_cs（LangGraph 原生） |
|---|---|
...
- **RAG 检索**（已完成）：agent 前置 `rag_node`，用 LangChain Retriever 检索知识库，做朴素 vs rerank 的指标对比（见上方「RAG 检索链路」）
- **多 Agent 路由 + HITL**（已完成）：`add_conditional_edges` 按意图路由到多个专职 Agent 节点 + 失败降级（低置信 + 运行时）+ `interrupt` 实现 human-in-the-loop 转人工（见上方路由图）
- **持久化 + 评测**（已完成）：`MemorySaver` → `SqliteSaver`（`CS_CHECKPOINT` 可切换，跨进程记忆）+ 本地端到端答案质量评测（DeepSeek judge）+ LangSmith 节点级 trace/数据集/evaluate（见上方「持久化 + 可观测/评测」）
- **Web 演示**（已完成）：FastAPI + 原生前端，SSE 流式 token、五阶段决策轨迹、坐席/审批模式、在飞流 abort
- **工具调用 + 人工审批**（已完成）：billing/technical 工具调用、ToolNode 回流、退款创建前人工审批、mock 业务库与离线工具测试
- **工具调用评测**（已完成）：should-call / 工具选择 / 参数准确率真实数字（含对抗集）
- **性能路由（扩展点）**：同类多 Agent 实例时按 `routing_score` 选最优（单实例场景暂不需要）
- **持久化（扩展点）**：`SqliteSaver` → `RedisSaver`/`PostgresSaver`（生产级、分布式，暂不做）
