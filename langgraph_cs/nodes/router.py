"""
route_by_intent —— 意图路由函数（conditional edges 的核心）。

graph.py 用 `add_conditional_edges("rag", route_by_intent, {...})` 把 rag 之后的流向
交给这个纯函数决定：读 state 里的 intent / confidence，返回**下一个节点的名字**。

路由分三层：
  1) 意图路由：technical / billing / escalation 映射到对应专职节点；
  2) 降级路由：confidence 低于阈值时一律落 general_agent——低置信不该交给专职 Agent
     自作主张，先用通用 Agent 稳妥应答（专职 Agent 运行时失败的降级放在各节点内部 try/except）；
  3) 升级路由：escalation 意图直达 escalation 节点（human-in-the-loop）。
  性能路由（同类多实例按 routing_score 选优）尚未实现，留作扩展点。

写成不依赖 LLM、无副作用的纯函数，可离线穷举各种 intent/confidence 组合做断言。
"""
import logging
