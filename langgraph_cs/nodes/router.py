...
对照 EchoMind 的 agent_orchestrator.py 三层路由：
  ...
  2) 降级路由（本函数实现的关键点）：confidence 低于阈值时，不管什么意图一律落到 general_agent
     ...
     （EchoMind 的"专职 Agent 运行时失败 -> 降级 general"那层，我们放在各专职节点内部 try/except。）
...
# 低于这个置信度就降级到 general_agent（教学阈值，对照 EchoMind 的低置信度降级）。
