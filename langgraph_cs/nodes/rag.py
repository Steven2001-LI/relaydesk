...
# 语料为条目级 chunk（百级规模，实数以 ingest 日志为准），运行期 k=5 的召回充分性
# 已由 eval/dataset.json 的检索评测验证；评测脚本会扫更大的 k（默认 10）放大"粗排→精排"差异。n≤k。
_RETRIEVE_K = 5
...
def _rerank_enabled() -> bool:
    ...
    评测时把 RAG_USE_RERANK 设成 0/false/no 即可关掉，对比朴素检索。
    """
...
def rag_node(state) -> dict:
    ...
    except Exception as ex:  # noqa: BLE001 统一兜底，避免整图崩
